...
DSMS_CONTEXT.mQPrefix = "SET PREFIX dsms: '" + DSMS_CONTEXT.mNs + "'; ";
...
DSMS_CONTEXT.queryMulti = function(pSqlArray, pResultHandler, pOptions)
{
  ...
  var lBatch = [DSMS_CONTEXT.mQPrefix];
...
DSMS_CONTEXT.resolveRefs = function(pObj, pProp, pCompletion)
{
  ...
  DSMS_CONTEXT.query("SELECT * WHERE afy:pinID IN (" + lRefs.map(function(_pR) { return "@" + _pR.ref; }).join(",") + ");", new QResultHandler(function(_pJson) { pCompletion(_pJson, lRefs); }, null, null));
}
...
{
  // Review: Why did I need to use 'CONTAINS' here? Why did '=' not work? Can afy:Classes be used as a family, or not?
  ...
  DSMS_CONTEXT.query("SELECT * FROM afy:Classes WHERE CONTAINS(afy:objectID, '" + lClassName + "');", new QResultHandler(lOnClassCount, null, null), {countonly:true});
}
DSMS_CONTEXT.createClasses = function(pCompletion)
{
  ...
  [
    ...
    "CREATE CLASS dsms:machines_rt AS SELECT * WHERE dsms:machine IN :0 AND dsms:run_id IN :1;",
    ...
    // "CREATE CLASS dsms:sin AS SELECT * WHERE dsms:sin_x IN :0;",
  ];
  var lPopulateSin =
    function()
    {
      var _lCompletion = function() { if (undefined != pCompletion) { pCompletion(); } }
      _lCompletion();
      /*
      var _lDoPopulate = function() { var _lS = []; for (var _i = 0; _i < 100; _i++) _lS.push("INSERT dsms:sin_x=" + _i + ", dsms:sin_val=" + Math.sin(Math.PI * 2 / (100 - _i))); DSMS_CONTEXT.queryMulti(_lS, new QResultHandler(_lCompletion, null, null)); }
      var _lOnCount = function(_pJson) { if (undefined == _pJson || parseInt(_pJson) == 0) { _lDoPopulate(); } else { _lCompletion(); } }
      DSMS_CONTEXT.query("SELECT * FROM dsms:sin;", new QResultHandler(_lOnCount, null, null), {countonly:true});
      */
    }
  var lProcess =
    function()
    {
      if (0 == lClassDecl.length) { lPopulateSin(); return; }
